from wills_pymol_crap.xyzMath import (Vec, Mat, Xform, RAD, projperp, SYMTET, SYMOCT, isvec, randnorm)
import itertools, re, os, inspect, random
      ...
      if self.kind[0] in "CD":
         ...
         if self.kind.startswith("D"):
            for i in range(self.nfold):
               ...
               CGO.extend([
                  pymol.cgo.CYLINDER, c1.x, c1.y, c1.z, c2.x, c2.y, c2.z, r, col[0], col[1], col[2], col[0], col[1],
                  col[2]
               ])
      elif self.kind == "T":
         cen = x * self.cen
...
class BuildCGO(object):
   ...
   def __init__(
      ...
      **kwargs,
      ...
      self.pdbout.write(f'CRYST1  {cell:7.3}  {cell:7.3}  {cell:7.3}  90.00  90.00  90.00 I 21 3        4')
